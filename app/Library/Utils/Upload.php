...
class Upload {
    ...
    private function setDriver($driver = null, $config = null){
        ...
        $config = $config ? : ($this->driverConfig ? : $this->config);
        $class = strpos($driver,'\\')? $driver : 'App\\Library\\Utils\Upload\\'.ucfirst(strtolower($driver));
        $this->uploader = new $class($config);
        if(!$this->uploader){
           return response(['success'=>false, 'message'=>'类文件不存在']);
        }
    }
    ...
    private function check($file) {
        ...
        if(!$this->checkLimitImage($file)) {
            $this->error = '上传文件限制了长宽';
            return false;
        }
    ...
    private function checkLimitImage($file)
    {
        if(substr($file['type'],0,4) != 'image') {
            return true;
        }
        if(empty($this->limitImage)) {
            return true;
        }

        $fileinfo = getimagesize($file['ext_name']);
        if($fileinfo[0] > $this->limitImage[0] || $fileinfo[1] > $this->limitImage[1])
        {
            return false;
        }
        return true;
