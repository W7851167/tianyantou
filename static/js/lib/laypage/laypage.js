...
Page.prototype.view = function(){
    var that = this, conf = that.config, view = [], dict = {}, type = that.type(), wrapper;
    ...
    conf.hideWrapper = conf.hideWrapper || 1;
    if(conf.groups > conf.pages){
        conf.groups = conf.pages;
    }
    if(type == 2) {
        wrapper = $(conf.cont).hide();
    }else if(type == 3){
        wrapper = conf.cont
    }else {
        wrapper = $('#'+conf.cont);
    ...
    wrapper.show();
    if(conf.pages <= 1 && conf.hideWrapper) {
        wrapper.hide();
        return ;
    }
